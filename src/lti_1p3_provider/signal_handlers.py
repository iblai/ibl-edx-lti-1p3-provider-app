...
def score_changed_handler(sender, **kwargs):  # pylint: disable=unused-argument
    ...
    """
    # NOTE: I think we need to do this more like edx originally did it in lti 1.1
    # NOTE: This would only work for grading a specific resource that was launched

    # TODO: Change to our checks
    lti_enabled = settings.FEATURES.get(
    ...
    try:
        ...
        log.debug(
            "LTI 1.3: Score Signal: Unknown resource, ignoring: kwargs=%s", kwargs
        )
    else:
        resource.update_score(weighted_earned, weighted_possible, modified)
        log.info("LTI 1.3: Score Signal: Grade upgraded: resource; %s", resource)
